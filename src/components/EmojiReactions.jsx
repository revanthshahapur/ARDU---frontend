...
const EmojiReactions = ({ postId, currentUser, token, onReaction }) => {
    ...
    const fetchReactionSummary = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/posts/${postId}/reactions/summary`, {
                headers: {
    ...
    const handleReaction = async (reactionType) => {
        try {
            ...
            const response = await fetch(`http://localhost:8080/api/posts/${postId}/reaction`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
